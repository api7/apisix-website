...
插件会增加 `/apisix/batch-requests` 这个接口，需要通过 [public-api](https://apisix.apache.org/docs/apisix/plugins/public-api) 插件来暴露它。
...
我们可以使用 [public-api](https://apisix.apache.org/docs/apisix/plugins/public-api) 插件轻易的设置自定义 uri。只需要在创建路由时设置需要的 uri 并改变 `public-api` 插件的配置即可。
...
首先，你需要为 batch request 的 API 设置一个路由，它将使用 [public-api](https://apisix.apache.org/docs/apisix/plugins/public-api) 插件。
